...
public class SpawnHighsprenProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity) {
		...
		if (world instanceof ServerLevel _level) {
			Entity entityToSpawn = StormlightModModEntities.HIGH_SPREN.get().spawn(_level, BlockPos.containing(x, y, z), MobSpawnType.MOB_SUMMONED);
		...
		if (entity instanceof ServerPlayer _player) {
			Advancement _adv = _player.server.getAdvancements().getAdvancement(new ResourceLocation("stormlight_mod:skybreakerlevel_1"));
